...
for A in ${A_choices[@]}; do
	for C in ${C_choices[@]}; do
		...
		nvcc -I build/include -L build/lib -O3 -arch=sm_75 sparsednn/driver_conv.cu -lcudnn -lcnpy -lcuda -DRESIDUAL=$RESIDUAL,HALF=0,A_Blocks=$A,C_Blocks=$C,Gy=$Gy,IC=$IC,OC=$OC,IMAGE_DIM=${IMAGE_DIM} -o ./exe
